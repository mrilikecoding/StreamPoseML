#!/bin/bash
# Script to fix imports by patching files

cd /Users/nathangreen/Development/stream_pose_ml

# Make a temporary directory to hold patched files
mkdir -p temp_fixes

# Fix blaze_pose_frame.py
cat > temp_fixes/blaze_pose_frame.py << 'EOF'
...
# Update the module's __init__.py file
cat > stream_pose_ml/__init__.py << 'EOF'
"""
...
from .stream_pose_client import StreamPoseClient
from .ml_flow_client import MLFlowClient
EOF

# Copy the patched files to their destinations
cp temp_fixes/blaze_pose_frame.py stream_pose_ml/blaze_pose/blaze_pose_frame.py

# Remove temporary directory 
rm -rf temp_fixes

# Create a conftest.py in the tests directory that handles imports
cat > stream_pose_ml/tests/conftest.py << 'EOF'
"""
...
# Create a file to explain how to run the tests
...
echo "Fixed imports. Now run the tests with 'python run_all_tests.py'"