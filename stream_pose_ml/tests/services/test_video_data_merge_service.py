...
class TestVideoDataMergeService:
    ...
    def mock_path_utility(self):
        ...
        with patch('stream_pose_ml.services.video_data_merge_service.path_utility') as mock_utility:
            # Set up mock returns for get_file_paths_in_directory
            mock_utility.get_file_paths_in_directory.side_effect = [
                ['/path/to/annotations/video1.json', '/path/to/annotations/video2.json'],  # annotations
                ['/path/to/videos/video1.webm', '/path/to/videos/video2.webm'],  # videos
                ['/path/to/sequences/video1_sequence.json', '/path/to/sequences/video2_sequence.json']  # sequences
            ]
            
            # Set up mock returns for get_file_name
            mock_utility.get_file_name.side_effect = lambda path, omit_extension=False: (
                'video1' if 'video1' in path else 'video2' if omit_extension else
                'video1.json' if 'video1' in path else 'video2.json'
            )
    ...
    def mock_transformer(self):
        ...
        with patch('stream_pose_ml.services.video_data_merge_service.AnnotationTransformerService') as mock_transformer:
    ...
    def mock_video_data_service(self):
        ...
        with patch('stream_pose_ml.services.video_data_merge_service.vds.VideoDataService') as mock_service:
            mock_instance = MagicMock()
    ...
    def mock_open_json(self):
        ...
        with patch('builtins.open', mock), patch('json.load') as mock_json_load:
            ...
            mock_json_load.side_effect = lambda f: (
                {"annotations": [{"label": "class1", "start_frame": 1, "end_frame": 5}]}  # annotation data
                if "annotation" in str(f.mock_calls) else
                {  # sequence data
                    "name": "video1.webm" if "video1" in str(f.mock_calls) else "video2.webm",
                    "frames": {
                        "1": {"frame_number": 1, "data": "test1"},
                        "2": {"frame_number": 2, "data": "test2"}
                    }
    ...
    def test_init_with_videos(self, mock_path_utility, mock_transformer):
        ...
        assert service.output_keypoints_path == (keypoints_path,)  # Note: This is a tuple in the implementation
    ...
    def test_create_video_annotation_map(self, mock_path_utility):
        ...
        assert service.annotation_video_map['/path/to/annotations/video1.json'] == '/path/to/videos/video1.webm'
        assert service.annotation_sequence_map['/path/to/annotations/video1.json'] == '/path/to/sequences/video1_sequence.json'
    ...
    def test_create_video_annotation_map_error(self):
        ...
        # Given/When/Then
        with pytest.raises(VideoDataMergeServiceError, match="There is no source video directory specified"):
            VideoDataMergeService(
    ...
    def test_generate_annotated_video_data_from_sequences(self, mock_path_utility, mock_transformer, mock_open_json):
        ...
        assert mock_transformer.return_value.update_video_data_with_annotations.call_count == 2

    def test_generate_annotated_video_data_from_videos(self, mock_path_utility, mock_transformer, mock_video_data_service, mock_open_json):
        ...
        with patch('stream_pose_ml.services.video_data_merge_service.os.path') as mock_path:
            mock_path.basename.side_effect = lambda path: path.split('/')[-1]
            mock_path.split.side_effect = lambda path: (path.rsplit('/', 1)[0], path.split('/')[-1])
    ...
    def test_generate_annotated_video_data_with_limit(self, mock_path_utility, mock_transformer, mock_open_json):
        ...
        assert mock_transformer.return_value.update_video_data_with_annotations.call_count == 1