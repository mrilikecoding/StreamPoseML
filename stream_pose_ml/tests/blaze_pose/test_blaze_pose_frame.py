...
def mock_transformer():
    ...
    with patch('stream_pose_ml.blaze_pose.blaze_pose_frame.OpenPoseMediapipeTransformer') as mock:
...
class TestBlazePoseFrameInitialization:
    ...
    def test_init_with_joints(self, sample_frame_data_with_joints):
        ...
        assert frame.joint_positions_raw == sample_frame_data_with_joints["joint_positions"]
    ...
    def test_init_with_generate_angles(self, sample_frame_data_with_joints, mock_transformer):
        """
        ...
        instance.open_pose_angle_definition_map.return_value = {"angle1": ("vector1", "vector2")}
        
        # Mock the generate_angle_measurements method to return a fake angles dict
        with patch.object(BlazePoseFrame, 'generate_angle_measurements', return_value={"angle1": MagicMock(spec=Angle)}):
            # Act
            frame = BlazePoseFrame(
                frame_data=sample_frame_data_with_joints, 
    ...
    def test_init_with_generate_distances(self, sample_frame_data_with_joints, mock_transformer):
        """
        ...
        # Arrange
        instance = mock_transformer.return_value
        instance.open_pose_distance_definition_map.return_value = {"distance1": ("joint1", "vector1")}
        
        # Mock the generate_distance_measurements method to return a fake distances dict
        with patch.object(BlazePoseFrame, 'generate_distance_measurements', return_value={"distance1": MagicMock(spec=Distance)}):
            # Act
            frame = BlazePoseFrame(
                frame_data=sample_frame_data_with_joints, 
...
class TestBlazePoseFrameJointMethods:
    ...
    def test_validate_joint_position_data_valid(self, sample_frame_data_with_joints):
        ...
        # Act & Assert
        assert frame.validate_joint_position_data(sample_frame_data_with_joints["joint_positions"]) is True
        
    def test_validate_joint_position_data_missing_joint(self, sample_frame_data_with_joints):
        """
        ...
        # Act & Assert
        with pytest.raises(BlazePoseFrameError, match=f"{first_joint} missing from joint positions dict"):
            frame.validate_joint_position_data(invalid_data)
            
    def test_validate_joint_position_data_missing_key(self, sample_frame_data_with_joints):
        """
    ...
    def test_set_joint_positions(self, sample_frame_data_with_joints):
        ...
        for joint in BlazePoseJoints:
            ...
            assert joint_positions[joint.name].x_normalized == source_data["x_normalized"]
            assert joint_positions[joint.name].y_normalized == source_data["y_normalized"]
            assert joint_positions[joint.name].z_normalized == source_data["z_normalized"]
    ...
    def test_get_average_joint(self, sample_frame_data_with_joints):
        ...
        if len(joint_names) >= 2:
            ...
            assert avg_joint.x_normalized == (frame.joints[joint1].x_normalized + frame.joints[joint2].x_normalized) / 2
            assert avg_joint.y_normalized == (frame.joints[joint1].y_normalized + frame.joints[joint2].y_normalized) / 2
            assert avg_joint.z_normalized == (frame.joints[joint1].z_normalized + frame.joints[joint2].z_normalized) / 2
...
class TestBlazePoseFrameMeasurements:
    ...
    def test_generate_angle_measurements_no_joints(self, sample_frame_data):
        ...
        # Act & Assert
        with pytest.raises(BlazePoseFrameError, match="There are no joint data to generate angles from"):
            frame.generate_angle_measurements({})
            
    def test_generate_angle_measurements(self, sample_frame_data_with_joints, mock_transformer):
        """
        ...
        angle_map = {
            "test_angle": (vector1_name, vector2_name)
        }
        ...
        with patch('stream_pose_ml.blaze_pose.blaze_pose_frame.Angle') as mock_angle:
            ...
            mock_angle.assert_called_once_with(
                "test_angle", 
                frame.vectors[vector1_name], 
                frame.vectors[vector2_name]
            )
    ...
    def test_generate_distance_measurements_no_joints(self, sample_frame_data):
        ...
        # Act & Assert
        with pytest.raises(BlazePoseFrameError, match="There are no joint data to generate distances from"):
            frame.generate_distance_measurements({})
            
    def test_generate_distance_measurements(self, sample_frame_data_with_joints, mock_transformer):
        """
        GIVEN a frame with joint positions and vectors
        WHEN generate_distance_measurements is called
        ...
        distance_map = {
            "test_distance": (joint_name, vector_name)
        }
        
        # Mock the Distance constructor
        with patch('stream_pose_ml.blaze_pose.blaze_pose_frame.Distance') as mock_distance:
...
class TestBlazePoseFrameIntegration:
    ...
    def test_full_frame_initialization_with_real_data(self, blaze_pose_sequence):
        ...
        if frame.has_joint_positions:
            ...
            assert all(isinstance(distance, Distance) for distance in frame.distances.values())