...
class TestBuildAndFormatDatasetJob:
    ...
    def mock_video_data_merge_service(self):
        ...
        with patch('stream_pose_ml.jobs.build_and_format_dataset_job.VideoDataMergeService') as mock_service:
    ...
    def mock_segmentation_service(self):
        ...
        with patch('stream_pose_ml.jobs.build_and_format_dataset_job.SegmentationService') as mock_service:
            mock_instance = MagicMock()
            mock_service.return_value = mock_instance
    ...
    def mock_dataset_serializer(self):
        ...
        with patch('stream_pose_ml.jobs.build_and_format_dataset_job.DatasetSerializer') as mock_serializer:
            mock_instance = MagicMock()
    ...
    def mock_round_nested_dict(self):
        ...
        with patch('stream_pose_ml.jobs.build_and_format_dataset_job.round_nested_dict') as mock_round:
            mock_round.side_effect = lambda item, precision: {
                **item,
                "features": {k: round(v, precision) if isinstance(v, float) else v 
    ...
    def test_format_dataset(self, mock_segmentation_service, mock_dataset_serializer, mock_round_nested_dict):
    ...
    def test_format_dataset_without_rounding(self, mock_segmentation_service, mock_dataset_serializer):
...
if str(project_root) not in sys.path:
    ...
    def test_write_dataset_to_csv_with_timestamp(self, mock_time, mock_pandas):
        ...
        mock_pandas.json_normalize.return_value.to_csv.assert_called_once_with(f"{csv_location}/dataset_123456789.csv")