...
function App() {
  ...
  useEffect(() => {
    if (isVideoStreamOn) {
      ...
      socketRef.current = io.connect(
        import.meta.env.VITE_STREAM_POSE_ML_API_ENDPOINT,
      );
  ...
  const DEVICE_SERVICE_UUID =
    import.meta.env.VITE_BLUETOOTH_DEVICE_SERVICE_UUID.toLowerCase();
  const DEVICE_CHARACTERISTIC_UUID =
    import.meta.env.VITE_BLUETOOTH_DEVICE_CHARACTERISTIC_UUID.toLowerCase();
  ...
  return (
    <div className="grid grid-cols-4 gap-4 p-6 content-stretch">
      <div className="prose col-span-full">
        <h1 className="">StreamPose ML Web Client</h1>
      </div>
      <div className="prose col-span-2">
        <div
          ...
          className={
            model
              ? "alert alert-success text-success-content"
              : "alert alert-warning text-warning-content"
          }
        >
          <div className="col-span-2">
            <ModelSelector setModel={setModel} />
            <h3>{model ? model : "No model selected."}</h3>
          </div>
        </div>
