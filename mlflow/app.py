...
def status():
    return jsonify({'status': 'running', 'message': 'MLflow model server is up and running.'}), 200

@app.route('/ping', methods=['POST'])
def ping():
    if model_server_process:
        return jsonify({'status': 'success', 'message': 'Model server is running.'}), 200
    else:
...
def invocations():
    ...
    try:
        ...
        return jsonify({'status': 'error', 'message': 'Failed to connect to model server.'}), 500
